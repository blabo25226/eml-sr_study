...
```bash
PYTHONPATH=src python -m eml_symbolic_regression.cli campaign proof-basin-probes --output-root artifacts/proof/v1.6/campaigns --label proof-basin-probes --overwrite
```
...
This campaign isolates the perturbed true-tree basin: 4/4 perturbed runs passed verifier-owned recovery, with 1 same-AST returns, 0 verified-equivalent returns, and 3 repaired candidates. Those outcomes demonstrate local basin and repair behavior, not blind-discovery performance.
