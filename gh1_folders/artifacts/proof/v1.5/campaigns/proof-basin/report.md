...
```bash
PYTHONPATH=src python -m eml_symbolic_regression.cli campaign proof-basin --output-root artifacts/proof/v1.5/campaigns --label proof-basin --overwrite
```
...
| Median runtime seconds | 0.02951 |
...
This campaign shows the strongest current behavior when the EML representation is verified after snapping: 9/9 runs passed verifier-owned recovery. Warm-start runs are especially useful evidence: 9 returned to the same compiled EML AST and 0 produced a different verified-equivalent AST. Those are not blind-discovery claims, but they are practical evidence that the paper's uniform EML tree can be compiled, embedded, perturbed, optimized, snapped, and independently verified.
